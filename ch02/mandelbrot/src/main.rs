...
fn main() {
    ...
    if args.len() != 5 {
        ...
        writeln!(std::io::stderr(), "Example: {} mandel.png 1000x750 -12.0,0.35 -1.0,0.20", args[0]).unwrap();
        std::process::exit(1);
    }

    let bounds = parse_pair(&args[2], 'x').expect("error parsing image dimensions");
    let upper_left = parse_complex(&args[3]).expect("error parsing uppler left corner point");
...
/// Try to determine if `c` is in the Mandelbrot set, useing at most `limit` iterations to decide.
///
...
/// string that can be parsed by `T::from_str`.
///
